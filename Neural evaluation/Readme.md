...
Natural intelligence is the intelligence possessed by humans and other animals. It is characterized by several key characteristics, including:

Recognition: the ability to identify patterns or stimuli.
Classification: the ability to assign stimuli to categories.
Response: the ability to select appropriate actions or responses based on stimuli.
...
A learning rule is a set of instructions that determines how the weights of a neural network should be adjusted based on the input data and the desired output. For example, the backpropagation algorithm is a learning rule that adjusts the weights based on the error between the predicted output and the actual output. Mathematical representations, such as signal graphs, sigma notation, and vector/matrix notation, can be used to represent and analyze neural network concepts.
